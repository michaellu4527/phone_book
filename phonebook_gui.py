...
def load_gui(self):
    ...
    '''
        Define the default tkinter widgets, their initial configuration,
        and place them using the grid geometry.

    '''

    # Creating and displaying the section headers (labels)
    self.lbl_fname = tk.Label(self.master, text='First Name:')  # Defining the label
    self.lbl_fname.grid(row=0, column=0, padx=(27, 0), pady=(10, 0), sticky=N + W)  # Placing the label
    ...
    # Creating and placing the text boxes
    ...
    # Bind, "listens" for an event to happen, then will execute the specified function when said event occurs
    # In this case, it is on selection of a listbox item.
    ...
    # Buttons for manipulating data in database or adding data to it
...
'''
    ...
    Don't want to execute anything here if the code accidentally gets here. The main code is run
    from the phonebook_main.py file
